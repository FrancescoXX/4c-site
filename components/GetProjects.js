...
const GetProjects = () => {
  ...
  function filterProjects(name) {
    name.target.classList.remove("bg-slate-600");
    const value = name.target;
    setFilter(value.textContent);
  }
          ...
          <div className="max-w-bodyContainer">
            ...
            <div className="mx-4 mt-5 mb-10 flex list-none flex-col items-center justify-center gap-[2rem] lg:flex-row lg:gap-[0.6rem]">
              {stacks.map((stack) => (
                <div
                  key={stack}
                  className={` w-[80%] select-none  rounded bg-white px-4 py-2 font-bold text-black transition-all ease-out hover:mx-2 hover:scale-110 hover:cursor-pointer hover:bg-slate-200 lg:w-[90%]`}
                  onClick={(stack) => filterProjects(stack)}
                >
                  {stack}
                </div>
              ))}
